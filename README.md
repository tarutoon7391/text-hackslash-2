...
- 2026-07-23 エンチャント抽選の統計検証（`npm run smoke:dungeon` セクション5・各レア度1000個以上生成）：
  **バグなし・GDD 10.2の仕様通り**を確認。実測値＝レア(枠1)0個率48.2%（理論50%）／エピック(枠2)平均1.004個（理論1.0）／
  レジェンダリー(枠3)平均1.500個（理論1.5）。部位別カテゴリ比率（武器=攻撃系65.8%等）も理論値±1pt以内。
  「エンチャントが付きにくい」という体感は仕様由来（雑魚ドロップの55%がコモン=枠0＋各枠50%充填で最低保証なし）。
